...
## 配图 / 画板
- **画板是默认动作**：结构（架构 / 流程 / 分类 / 时间线）、对比（A vs B）、光谱（程度递进）、评分（多维评估）都配可编辑画板，并直接插进文档对应小节。
- 用 `beautiful-feishu-whiteboard`（配色风格）+ `lark-whiteboard`（写进文档）配合；完整工作流、配图映射、渲染踩坑见 `references/whiteboards.md`。
- 产品 / 界面：抓**官方截图**随文附上。
- 临时文件用完即清理。
